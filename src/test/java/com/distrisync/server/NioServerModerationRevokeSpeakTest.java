...
class NioServerModerationRevokeSpeakTest {

    @Test
    void remoteMailboxRevokeSpeak_unicastsRoleUpdateAndClearsSpeakBit() throws Exception {
        ...
        ClientSession target = sessionFor("member-client", RoomPermissions.MEMBER);
        ...
        try {
            ...
            processRemoteRevokeSpeak(server, "member-client");

            assertThat(target.permissions).isEqualTo(RoomPermissions.PERM_DRAW);
            assertThat(RoomPermissions.canSpeak(target.permissions)).isFalse();
            ...
            assertThat(payload.newPermissions()).isEqualTo(RoomPermissions.PERM_DRAW);
            assertThat(RoomPermissions.canSpeak(payload.newPermissions())).isFalse();
        } finally {
            client.close();
            selector.close();
            acceptor.close();
    ...
    @Test
    void remoteMailboxRevokeSpeak_broadcastsRoleUpdateWhenTargetNotLocal() throws Exception {
        String localNodeId = "local-node";
        RoomManager roomManager = new RoomManager();
        NioServer server = new NioServer(0, roomManager, null, null, null, localNodeId);
        ...
        try {
            ...
            processRemoteRevokeSpeak(server, "remote-member");
    ...
    private static void processRemoteRevokeSpeak(NioServer server, String targetClientId) {
        Message revoke = new Message(
                MessageType.MODERATION_ACTION,
                MessageCodec.gson().toJson(new MessageCodec.ModerationActionPayload(
                        "REVOKE_SPEAK", targetClientId, "muted by moderator")));
        ByteBuffer encoded = MessageCodec.encode(revoke);
